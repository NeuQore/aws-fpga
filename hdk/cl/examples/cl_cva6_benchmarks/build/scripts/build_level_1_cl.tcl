...
###############################################################################
# cl_hbm_mmcm defaults to 450 MHz (CLKFBOUT_MULT_F=23.625, DIVCLK_DIVIDE=2,
# CLKOUT0_DIVIDE_F=2.625). First DCP WNS=-0.332 ns on SmartConnect in
# clk_out1_cl_hbm_mmcm (period 2.222 ns). AWS_CLK_GEN recipes do not apply
# here. Keep VCO, set CLKOUT0_DIVIDE_F=4.000 => 295.3125 MHz (period 3.390 ns).
###############################################################################
print "Retuning HBM AXI MMCM from 450 MHz to 295.3125 MHz"
set hbm_mmcm [get_cells -quiet \
  WRAPPER/CL/CL_HBM/HBM_PRESENT_EQ_1.HBM_WRAPPER_I/HBM_MMCM_I/inst/mmcme4_adv_inst]
if {$hbm_mmcm eq ""} {
    # #region agent log
    _agent_dbg "H" "build_level_1_cl.tcl:hbm_mmcm" "HBM MMCM cell missing" {{"found":false}}
    # #endregion
    error "HBM MMCM cell not found; cannot retune HBM AXI clock"
}
set old_div [get_property CLKOUT0_DIVIDE_F $hbm_mmcm]
set_property CLKOUT0_DIVIDE_F 4.000 $hbm_mmcm
set new_div [get_property CLKOUT0_DIVIDE_F $hbm_mmcm]
print "HBM MMCM CLKOUT0_DIVIDE_F ${old_div} -> ${new_div} (295.3125 MHz)"
# #region agent log
_agent_dbg "H" "build_level_1_cl.tcl:hbm_mmcm" "retuned HBM AXI MMCM" \
  [format {{"cell":"%s","old_div":"%s","new_div":"%s"}} $hbm_mmcm $old_div $new_div]
# #endregion
...
# #region agent log
_agent_dbg "E" "build_level_1_cl.tcl:post_route" "post-route timing check" \
  [format {{"failPath":%s}} $failPath]
# #endregion
if {$failPath>0} {
