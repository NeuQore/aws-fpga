...
# #region agent log
proc _agent_dbg {hid loc msg data} {
    if {[catch {
        set fp [open "/projects/prj1/sle-wajahat/.cursor/debug-76b74b.log" a]
        set ts [clock milliseconds]
        puts $fp [format {{"sessionId":"76b74b","hypothesisId":"%s","location":"%s","message":"%s","data":%s,"timestamp":%s}} $hid $loc $msg $data $ts]
        close $fp
    }]} {}
}
_agent_dbg "A" "build_all.tcl:env" "HDK env at Tcl start" [format {{"HDK_SHELL_DIR_set":%s,"CL_DIR_set":%s,"clock_recipe_hbm":"%s","CL":"%s"}} [info exists ::env(HDK_SHELL_DIR)] [info exists ::env(CL_DIR)] $clock_recipe_hbm $CL]
# #endregion
if { [info exists ::env(HDK_SHELL_DIR)] } {
    set HDK_SHELL_DIR $::env(HDK_SHELL_DIR)
} else {
    # #region agent log
    _agent_dbg "A" "build_all.tcl:hdk_missing" "HDK_SHELL_DIR not set" {{"fatal":true}}
    # #endregion
