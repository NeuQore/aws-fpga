...
def _agent_dbg(hypothesis_id, location, message, data=None):
    try:
        with open("/projects/prj1/sle-wajahat/.cursor/debug-76b74b.log", "a") as _f:
            _f.write(json.dumps({
                "sessionId": "76b74b",
...
def generate_dcp_tarball(cl, tag, clock_recipe_a, clock_recipe_b, clock_recipe_c, clock_recipe_hbm):
    ...
    if os.path.exists(dcp_violated_file) or os.path.exists(dcp_file):
        ...
        # #region agent log
        _agent_dbg("E", "aws_build_dcp_from_cl.py:dcp_missing", "post-route DCP not found", {
            "checkpoints_dir": checkpoints_dir,
            "dcp_file": dcp_file,
            "dcp_violated_file": dcp_violated_file,
            "dcp_exists": os.path.exists(dcp_file),
            "violated_exists": os.path.exists(dcp_violated_file),
            "checkpoint_names": os.listdir(checkpoints_dir) if os.path.isdir(checkpoints_dir) else [],
        })
        # #endregion
...
def main():
    ...
    # #region agent log
    _agent_dbg("A", "aws_build_dcp_from_cl.py:env", "HDK/CL env before Vivado", {
        "argv": sys.argv,
        "CL_DIR": os.getenv("CL_DIR"),
        "HDK_SHELL_DIR": os.getenv("HDK_SHELL_DIR"),
        "HDK_SHELL_DESIGN_DIR": os.getenv("HDK_SHELL_DESIGN_DIR"),
        "HDK_COMMON_DIR": os.getenv("HDK_COMMON_DIR"),
        "CVA6_REPO_DIR": os.getenv("CVA6_REPO_DIR"),
        "TARGET_CFG": os.getenv("TARGET_CFG"),
        "VIVADO_TOOL_VERSION": os.getenv("VIVADO_TOOL_VERSION"),
        "AWS_FPGA_REPO_DIR": os.getenv("AWS_FPGA_REPO_DIR"),
        "cwd": os.getcwd(),
        "aws_clk_gen": bool(options.aws_clk_gen),
        "clock_recipe_hbm": options.clock_recipe_hbm,
        "clock_recipe_in_argv": "clock_recipe" in str(sys.argv),
    })
    # #endregion
    ...
    if not options.aws_clk_gen and "clock_recipe" in str(sys.argv):
        # #region agent log
        _agent_dbg("B", "aws_build_dcp_from_cl.py:clk_gate", "blocked: clock_recipe without --aws_clk_gen", {
            "argv": sys.argv,
            "aws_clk_gen": False,
            "clock_recipe_hbm": options.clock_recipe_hbm,
        })
    ...
    if options.package_only:
        ...
        # #region agent log
        _agent_dbg("A", "aws_build_dcp_from_cl.py:vivado_cmd", "launching Vivado", {
            "cmd": cmd,
            "build_tag": build_tag,
        ...
        rc = os.system(cmd)
        # #region agent log
        _agent_dbg("E", "aws_build_dcp_from_cl.py:vivado_rc", "Vivado process returned", {
            "rc": rc,
            "build_tag": build_tag,
        })
        # #endregion
