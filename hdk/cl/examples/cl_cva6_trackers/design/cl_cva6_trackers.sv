...
//====================================================================================
// Top level module file for cl_cva6_trackers
//====================================================================================

module cl_cva6_trackers
    #(
      parameter EN_DDR = 0,
      parameter EN_HBM = 1
    )
...
`include "cl_cva6_trackers_defines.vh"
...
//=============================================================================
  ...
  localparam logic [7:0] REG_TRACE_CTRL      = `ADDR_TRACE_CTRL;
  localparam logic [7:0] REG_TRACE_STATUS    = `ADDR_TRACE_STATUS;
  localparam logic [7:0] REG_TRACE_START_LO  = `ADDR_TRACE_START_LO;
  localparam logic [7:0] REG_TRACE_START_HI  = `ADDR_TRACE_START_HI;
  localparam logic [7:0] REG_TRACE_END_LO    = `ADDR_TRACE_END_LO;
  localparam logic [7:0] REG_TRACE_END_HI    = `ADDR_TRACE_END_HI;
  localparam logic [7:0] REG_TRACE_CYCLE_LO  = `ADDR_TRACE_CYCLE_LO;
  localparam logic [7:0] REG_TRACE_CYCLE_HI  = `ADDR_TRACE_CYCLE_HI;
  localparam logic [7:0] REG_TRACE_INSTR_CNT = `ADDR_TRACE_INSTR_CNT;
  localparam logic [7:0] REG_TRACE_IC_CNT    = `ADDR_TRACE_IC_CNT;
  localparam logic [7:0] REG_TRACE_DC_CNT    = `ADDR_TRACE_DC_CNT;
  localparam logic [7:0] REG_TRACE_SEL       = `ADDR_TRACE_SEL;
  localparam logic [7:0] REG_TRACE_IDX       = `ADDR_TRACE_IDX;
  localparam logic [7:0] REG_TRACE_WORD      = `ADDR_TRACE_WORD;
  localparam logic [7:0] REG_TRACE_RDATA_LO  = `ADDR_TRACE_RDATA_LO;
  localparam logic [7:0] REG_TRACE_RDATA_HI  = `ADDR_TRACE_RDATA_HI;
  localparam logic [7:0] REG_TRACE_EVT_CNT   = `ADDR_TRACE_EVT_CNT;
  localparam logic [7:0] REG_TRACE_L1_CNT    = `ADDR_TRACE_L1_CNT;
  localparam logic [7:0] REG_TRACE_IC_HIT    = `ADDR_TRACE_IC_HIT;
  localparam logic [7:0] REG_TRACE_IC_MISS   = `ADDR_TRACE_IC_MISS;
  localparam logic [7:0] REG_TRACE_DC_HIT    = `ADDR_TRACE_DC_HIT;
  localparam logic [7:0] REG_TRACE_DC_MISS   = `ADDR_TRACE_DC_MISS;

  logic        cpu_run;
  logic        trace_enable;
  logic        trace_clear;
  logic        trace_stop_on_excp;
  logic [63:0] trace_start;
  logic [63:0] trace_end;
  logic [2:0]  trace_sel;
  logic [15:0] trace_idx;
  logic [3:0]  trace_word;
  logic [63:0] trace_rdata;
  logic [63:0] trace_cycle;
  logic [31:0] trace_instr_count;
  logic [31:0] trace_icache_count;
  logic [31:0] trace_dcache_count;
  logic [31:0] trace_event_count;
  logic [31:0] trace_l1_count;
  logic [31:0] trace_ic_hit_count;
  logic [31:0] trace_ic_miss_count;
  logic [31:0] trace_dc_hit_count;
  logic [31:0] trace_dc_miss_count;
  logic [31:0] trace_status;
  logic [4:0]  clear_hold;
  ...
  always_ff @(posedge clk_main_a0 or negedge rst_main_n) begin
    if (!rst_main_n) begin
      ...
      trace_enable     <= 1'b0;
      trace_clear      <= 1'b0;
      trace_stop_on_excp <= 1'b0;
      trace_start      <= 64'd2000;
      trace_end        <= 64'd4000;
      trace_sel        <= 3'd0;
      trace_idx        <= 16'd0;
      trace_word       <= 4'd0;
      ...
      uart_rx_pop    <= 1'b0;
      if (clear_hold != 5'd0)
        clear_hold <= clear_hold - 5'd1;
      trace_clear    <= (clear_hold != 5'd0) ||
                        ((next_state == WRITE) &&
                         ((addr_wr_hs ? sh_ocl_awaddr_q[7:0] : write_addr[7:0]) == REG_TRACE_CTRL) &&
                         sh_ocl_wdata_q[1]);
      ...
      if (next_state == WRITE) begin
        unique case ((addr_wr_hs ? sh_ocl_awaddr_q[7:0] : write_addr[7:0]))
          ...
          REG_TRACE_CTRL: begin
            trace_enable <= sh_ocl_wdata_q[0];
            trace_stop_on_excp <= sh_ocl_wdata_q[2];
            if (sh_ocl_wdata_q[1])
              clear_hold <= 5'd16;
          end
          REG_TRACE_START_LO: trace_start[31:0]  <= sh_ocl_wdata_q;
          REG_TRACE_START_HI: trace_start[63:32] <= sh_ocl_wdata_q;
          REG_TRACE_END_LO:   trace_end[31:0]    <= sh_ocl_wdata_q;
          REG_TRACE_END_HI:   trace_end[63:32]   <= sh_ocl_wdata_q;
          REG_TRACE_SEL:      trace_sel          <= sh_ocl_wdata_q[2:0];
          REG_TRACE_IDX:      trace_idx          <= sh_ocl_wdata_q[15:0];
          REG_TRACE_WORD:     trace_word         <= sh_ocl_wdata_q[3:0];
          default: ;
        endcase
      end
      ...
      if (addr_rd_hs) begin
        ...
        unique case (sh_ocl_araddr_q[7:0])
          ...
          REG_TRACE_CTRL:     ocl_sh_rdata_q <= {29'b0, trace_stop_on_excp, trace_clear, trace_enable};
          REG_TRACE_STATUS:   ocl_sh_rdata_q <= trace_status;
          REG_TRACE_START_LO: ocl_sh_rdata_q <= trace_start[31:0];
          REG_TRACE_START_HI: ocl_sh_rdata_q <= trace_start[63:32];
          REG_TRACE_END_LO:   ocl_sh_rdata_q <= trace_end[31:0];
          REG_TRACE_END_HI:   ocl_sh_rdata_q <= trace_end[63:32];
          REG_TRACE_CYCLE_LO: ocl_sh_rdata_q <= trace_cycle[31:0];
          REG_TRACE_CYCLE_HI: ocl_sh_rdata_q <= trace_cycle[63:32];
          REG_TRACE_INSTR_CNT: ocl_sh_rdata_q <= trace_instr_count;
          REG_TRACE_IC_CNT:    ocl_sh_rdata_q <= trace_icache_count;
          REG_TRACE_DC_CNT:    ocl_sh_rdata_q <= trace_dcache_count;
          REG_TRACE_EVT_CNT:   ocl_sh_rdata_q <= trace_event_count;
          REG_TRACE_L1_CNT:    ocl_sh_rdata_q <= trace_l1_count;
          REG_TRACE_IC_HIT:    ocl_sh_rdata_q <= trace_ic_hit_count;
          REG_TRACE_IC_MISS:   ocl_sh_rdata_q <= trace_ic_miss_count;
          REG_TRACE_DC_HIT:    ocl_sh_rdata_q <= trace_dc_hit_count;
          REG_TRACE_DC_MISS:   ocl_sh_rdata_q <= trace_dc_miss_count;
          REG_TRACE_SEL:       ocl_sh_rdata_q <= {29'b0, trace_sel};
          REG_TRACE_IDX:       ocl_sh_rdata_q <= {16'b0, trace_idx};
          REG_TRACE_WORD:      ocl_sh_rdata_q <= {28'b0, trace_word};
          REG_TRACE_RDATA_LO:  ocl_sh_rdata_q <= trace_rdata[31:0];
          REG_TRACE_RDATA_HI:  ocl_sh_rdata_q <= trace_rdata[63:32];
...
//=============================================================================
  ...
  cva6_f2_soc #(
    ...
    .trace_rd_clk_i        (clk_main_a0),
    .trace_enable_i        (trace_enable),
    .trace_start_i         (trace_start),
    .trace_end_i           (trace_end),
    .trace_clear_i         (trace_clear),
    .trace_stop_on_excp_i  (trace_stop_on_excp),
    .trace_sel_i           (trace_sel),
    .trace_idx_i           (trace_idx),
    .trace_word_i          (trace_word),
    .trace_rdata_o         (trace_rdata),
    .trace_cycle_o         (trace_cycle),
    .trace_instr_count_o   (trace_instr_count),
    .trace_icache_count_o  (trace_icache_count),
    .trace_dcache_count_o  (trace_dcache_count),
    .trace_event_count_o   (trace_event_count),
    .trace_l1_count_o      (trace_l1_count),
    .trace_ic_hit_count_o  (trace_ic_hit_count),
    .trace_ic_miss_count_o (trace_ic_miss_count),
    .trace_dc_hit_count_o  (trace_dc_hit_count),
    .trace_dc_miss_count_o (trace_dc_miss_count),
    .trace_status_o        (trace_status)
  );
